...
export const PeriodEvolutionChart: React.FC<PeriodEvolutionChartProps> = ({
  ...
  const maxSpend = Math.max(...periods.map((p) => p.totalSpend || 0), 100);
  ...
  const activePeriod = periods.find((p) => p.id === (hoveredPeriodId || selectedPeriodId));

  return (
    <div className="flex flex-col gap-3 w-full">
      {activePeriod && (
        <div className="flex items-center justify-between rounded-lg border border-border-subtle/80 bg-bg-secondary px-3.5 py-2 text-xs transition-all shadow-2xs">
      ...
      <div className="relative flex h-72 w-full pt-4 pb-8 pl-10 pr-2">
        <div className="absolute inset-y-4 left-0 flex flex-col justify-between text-right text-[10px] font-mono text-text-muted pr-2 select-none">
          {yTicks.map((tick, i) => (
            ...
            return (
              ...
              >
                ...
                <div className="absolute -bottom-6 flex flex-col items-center">
                  <span
                    className={`font-mono text-[10px] tracking-tight uppercase transition-colors ${
                      isSelected
