...
const AiHealthAnalytics: React.FC<AiHealthAnalyticsProps> = ({
  ...
  const severityColor = (severity?: string) => {
    if (severity === "good") return "text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border-emerald-500/30";
    if (severity === "warning") return "text-rose-600 dark:text-rose-400 bg-rose-500/10 border-rose-500/30";
      ...
      <div className="flex flex-col gap-6 animate-[fadeIn_0.4s_cubic-bezier(0.16,1,0.3,1)_forwards] w-full">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 px-1">
          ...
          <span className="text-[11.5px] text-text-muted italic">
            Values shown below are illustrative placeholders, not your real finances.
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="rounded-2xl border border-border-subtle bg-bg-card p-5 shadow-subtle flex flex-col justify-between gap-4">
            ...
            <div className="rounded-xl border border-dashed border-border-subtle bg-bg-primary/40 p-3 flex items-center justify-between text-xs">
              ...
              <span className="font-mono font-semibold text-text-primary">Balanced · 0% Unaccounted</span>
            </div>
          </div>
        ...
        <div className={`${STAT_CARD} border-t-2 border-t-border-subtle`}>
          ...
          <span className={STAT_SUBTEXT}>
            {isBehindTarget
              ...
              : todaySpent > 0
              ? `Spent ${currency}${Math.round(todaySpent).toLocaleString("en-IN")} today of ${currency}${Math.floor(grossWeekLimit).toLocaleString("en-IN")} ${daysThisWeek}-day budget`
              : `Spend max ${currency}${Math.floor(grossDailyLimit)}/day for next ${daysThisWeek} days`}
          </span>
        </div>
                  ...
                  <tbody className="divide-y divide-border-subtle">
                    {cycleHistory.slice(0, 5).map((c) => (
                      <tr key={c.startStr}>
                        ...
                        <td className={`py-2 text-right font-mono font-semibold ${c.savings >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`}>
