    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl rounded-sm border border-border-subtle bg-bg-card p-6 shadow-2xl overflow-hidden max-h-[85vh] flex flex-col apple-modal-spring">
        <div className="mb-6 flex items-center justify-between">
          ...
          <button onClick={onClose} className="rounded-xs p-2 text-text-secondary hover:bg-bg-primary hover:text-text-primary transition-colors cursor-pointer">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
          {duplicateGroups.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-sm bg-green-500/10 text-green-500">
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-2 rounded-sm bg-orange-500/10 p-3 text-orange-500 mb-2">
                ...
                <div key={key} className="rounded-sm border border-border-subtle/50 surface-container-subtle p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      ...
                      className="rounded-xs border border-text-primary bg-text-primary px-3 py-1.5 text-xs font-mono uppercase tracking-wider font-semibold text-bg-primary transition-all hover:opacity-90 disabled:opacity-50 cursor-pointer"
                    >
                  ...
                  <div className="space-y-2">
                    {items.map((item) => (
                      <div key={item.id} className="flex items-center justify-between rounded-xs bg-bg-card px-3 py-2 text-[13px] border border-border-subtle/50">
                        <div className="flex items-center gap-3">
                          {item.coverImage ? (
                            <img src={item.coverImage} alt="Cover" className="h-8 w-6 rounded-xs object-cover" />
                          ) : (
                            <div className="h-8 w-6 rounded-xs bg-bg-primary" />
                          )}
                        ...
                        <div className="flex gap-2">
                          {item.anilistId && <span className="rounded-xs bg-[#2b2d42] px-1.5 py-0.5 text-[10px] font-mono font-bold uppercase text-white">AL</span>}
                          {item.traktId && <span className="rounded-xs bg-red-600 px-1.5 py-0.5 text-[10px] font-mono font-bold uppercase text-white">TRK</span>}
                        </div>
                      </div>
                    ))}
                  </div>
